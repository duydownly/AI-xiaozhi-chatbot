...
void OttoEmojiDisplay::SetupChatLabel() {
    ...
    // Use bottom bar if available to keep chat at the bottom; otherwise fallback to container_
    lv_obj_t* parent = bottom_bar_ ? bottom_bar_ : container_;

    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    auto text_font = lvgl_theme->text_font()->font();

    // Create the chat message label as the shared chat_message_label_ used by LcdDisplay::SetChatMessage
    chat_message_label_ = lv_label_create(parent);
    lv_label_set_text(chat_message_label_, "");

    if (parent == bottom_bar_) {
        // Style similar to LcdDisplay bottom_bar_
        ...
        // Fallback behavior (original Otto): centered, scroll circular
        lv_obj_set_width(chat_message_label_, width_ * 0.9);                        // 90% of screen width
    ...
    // Apply theme to ensure fonts/colors updated
